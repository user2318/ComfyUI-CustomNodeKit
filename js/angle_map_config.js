...
// 全局样式（只注入一次）
if (!window.__angleMapStyleInjected) {
    window.__angleMapStyleInjected = true;
...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        nodeType.prototype.onNodeCreated = function () {
            ...
            if (angleMapWidget) {
                ...
                if (inputEl && inputEl.parentNode) {
                    ...
                    // 如 inputEl 不可用，通过 addDOMWidget 添加到节点
                    try {
                        this.addDOMWidget("angle_map_actions", "custom", btnContainer, { serialize: false });
                    } catch (e) {
                        // addDOMWidget 也可能失败，记录日志即可
                        console.debug("[AngleMapConfig] addDOMWidget failed:", e);
                    }
                }
            }
        ...
        // 获取参考图数量
        nodeType.prototype.getReferenceCount = function () {
            const refInput = this.inputs?.find(inp => inp.name === "reference_image");
        ...
        nodeType.prototype.openAngleMapUI = function () {
            ...
            this.showAngleMapCanvas(count, existingAngles, angleMapWidget);
        };

        // 椭圆角度映射画布
        nodeType.prototype.showAngleMapCanvas = function (pointCount, existingAngles, angleMapWidget) {
            ...
            // 角度映射：角度角度制，0=底部(正面)，顺时针正，逆时针负
            // 底部 0°, 右 +90°, 顶部 ±180°, 左 -90°
            // 点位置: 角度转换为椭圆上的位置
            let points = [];
            if (existingAngles && existingAngles.length === pointCount) {
                ...
                for (let i = 0; i < pointCount; i++) {
                    ...
                    const angle = frac * 360 - 180; // 从 -180 到 180
                    points.push(angle);
                }
            }
            ...
            const posToAngle = (px, py) => {
                // 像素坐标转角度
                const dx = px - cx;
                const dy = py - cy;  // 正值 = 底部 (0°正面)，负值 = 顶部 (±180°背面)
            ...
            document.getElementById("angle-confirm-btn").onclick = () => {
                ...
                const jsonStr = JSON.stringify(angles);
                if (angleMapWidget) {
